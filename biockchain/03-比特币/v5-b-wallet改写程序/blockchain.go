...
func NewBlockChain() *BlockChain {
	...
	_ = db.Update(func(tx *bolt.Tx) error {
		...
		if b == nil {
			...
			//写入创世块
			//创建一个挖矿交易，里面写入创世语
			coinbaseTx := NewCoinbaseTx("1Fakfxjba4LwEtNVUJnz9erXqjgBeRzvuz", genesisInfo)
...
func (bc *BlockChain) FindMyUtxo(pubKeyHash []byte) []UtxoInfo {
	fmt.Printf("FindMyUtxo called, address:%x\n", pubKeyHash)
...
func (bc *BlockChain) FindNeedUtxoInfo(pubKeyHash []byte, amount float64) ([]UtxoInfo, float64) {
	fmt.Printf("FindNeedUtxoInfo called, pubKeyHash :%x, amount:%f\n", pubKeyHash, amount)
